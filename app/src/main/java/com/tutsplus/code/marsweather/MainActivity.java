...
public class MainActivity extends Activity {
    ...
    private MarsWeather helper = MarsWeather.getInstance();
    private int today = Calendar.getInstance().get(Calendar.DAY_OF_MONTH);
    private int mainColor = Color.parseColor("#FF5722");
    private SharedPreferences mSharedPref;

    private static final String SHARED_PREFS_IMG_KEY = "img";
    private static final String  IMAGES_API_ENDPOINT = "https://api.flickr.com/services/rest/?" +
            "format=json&" +
            "nojsoncallback=1&" +
            "sort=random&method=flickr.photos.search&" +
            "tags=mars,planet,rover&" +
            "tag_mode=all&api_key=";

    private static final String  SHARED_PREFS_DAY_KEY = "day";
    private static final String FLICKR_API_KEY = "2488a19691d2bc9a99061cb13910c9bb";
    private static final String    RECENT_API_ENDPOINT = "http://marsweather.ingenology.com/v1/latest/";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                mTypeface = Typeface.createFromAsset(getAssets(), "fonts/Lato-light.ttf");
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mTxtDegrees.setTypeface(mTypeface);
                        mTxtWeather.setTypeface(mTypeface);
                    }
                });
            }
        }).start();
